"""
syphilis_ed_cea.py  ·  Emergency Department Universal Syphilis Screening — CEA  (v3)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
v3 changes vs v2
────────────────
1. POC test cost ($50/screen)  Initial treponemal point-of-care screen added to program costs.
   Applies to all extra screens above the baseline strata-weighted usual-care coverage.

2. Social worker cost ($500/case)  Per-treated-case care coordination added to program costs.
   Reflects the ED social worker / nurse navigator needed to ensure penicillin completion.

3. US Life Table 2021 in infant Markov  Age-specific qx values replace the fixed mu_bg=0.003
   constant. Mortality at age 0 (qx=0.00586) is ~2× the prior fixed rate; by age 2 it falls
   to 0.00027, and climbs again from age 20+. This produces more accurate DALY accumulation.

4. Separate IUFD cost ($13,049 CPI-adj) vs term stillbirth ($141,792 CPI-adj)  The iufd_subset
   (≥28w intrauterine fetal deaths) is now costed at the correct lower rate. Non-IUFD stillbirths
   continue to use co.sb.  Prevents overestimation of savings for early fetal losses.

5. Preterm delivery cost ($37,780 CPI-adj) added to savings  Preterm births averted by the
   intervention generate cost savings over a term delivery. Previously tracked but never costed.

6. Term delivery cost ($13,828 CPI-adj)  Added to Costs for reference and UI display of
   net-of-delivery savings.

7. NICU cost ($50,000) as a separate savings line for CS complicated infants  Added alongside
   cs_wu (the diagnostic workup cost).  CS complicated neonates require NICU admission beyond
   the acute workup captured by cs_wu.

8. CS natural history parameters  Three rates from the clinical literature added to INFANT_MK
   and the sidebar:  cs_early_cure_rate (0.95), cs_late_manifest_rate (0.20),
   cs_neuro_disorder_rate (0.20).  Used for Markov calibration checks in Tab 4.

v2 changes vs v1  (retained for reference)
────────────────
1. Infant lifetime Markov  (4 states: Healthy / Mild-seq / Severe-seq / Dead)
2. Mutual exclusivity fix  Sequential conditional birth-outcome hierarchy.
3. Pregnancy detection  p_identified applied multiplicatively to coverage.
4. Serofast parameter fix  p_trepo_sf and p_ux_sf passed explicitly everywhere.
5. cs_lt removed from Costs  Long-term CS costs produced by infant Markov.
...
# ── US Life Table 2021 (CDC NCHS) — age-specific annual mortality (qx) ───────
# Used in the infant Markov module to replace the fixed mu_bg constant.
# Index = age in years (0–109); ages 110+ use the last available rate.
LIFE_TABLE_QX: List[float] = [
    0.00586,  0.00042,  0.000272, 0.000225, 0.000184, 0.000157, 0.00014,  0.000128,  # 0–7
    0.000122, 0.000123, 0.000129, 0.000138, 0.000164, 0.00022,  0.00031,  0.000446,  # 8–15
    0.000637, 0.000868, 0.0011,   0.00127,  0.001373, 0.001488, 0.001605, 0.001714,  # 16–23
    0.001835, 0.001963, 0.002082, 0.002202, 0.00233,  0.002457, 0.002574, 0.002683,  # 24–31
    0.002787, 0.002881, 0.002974, 0.003074, 0.003175, 0.003295, 0.003444, 0.003608,  # 32–39
    0.00378,  0.003958, 0.004144, 0.004337, 0.00454,  0.004774, 0.005064, 0.005399,  # 40–47
    0.005796, 0.006214, 0.006671, 0.007167, 0.007736, 0.008351, 0.009035, 0.00977,   # 48–55
    0.010567, 0.011398, 0.012291, 0.013224, 0.014267, 0.015353, 0.016484, 0.017617,  # 56–63
    0.018759, 0.019914, 0.021104, 0.022423, 0.023847, 0.025357, 0.02705,  0.02897,   # 64–71
    0.031188, 0.033754, 0.036747, 0.040563, 0.044308, 0.048498, 0.053229, 0.058778,  # 72–79
    0.064617, 0.070947, 0.077834, 0.085686, 0.094809, 0.10509,  0.116592, 0.129306,  # 80–87
    0.142732, 0.157638, 0.174458, 0.193027, 0.21293,  0.232657, 0.251826, 0.270943,  # 88–95
    0.289756, 0.307998, 0.325393, 0.341662, 0.358746, 0.376683, 0.395517, 0.415293,  # 96–103
    0.436058, 0.45786,  0.480753, 0.504791, 0.530031, 0.556532,                       # 104–109
]

def lt_qx(age: int) -> float:
    """Return age-specific annual mortality probability from the 2021 US Life Table."""
    if age < 0:       return 0.0
...
INFANT_MK = {
    ...
    "mu_bg":             0.003,   # legacy constant — superseded by LIFE_TABLE_QX in v3
    "q_progress":        0.002,   # annual P(mild sequelae → severe) [calibrated: ~20% lifetime]
    # CS natural history (clinical literature; used for Markov calibration checks)
    "cs_early_cure_rate":    0.95,  # P(cure | CS detected + treated in first month; Sheffield 2002)
    "cs_late_manifest_rate": 0.20,  # P(late complications | untreated CS survivor; CDC 2023)
    "cs_neuro_disorder_rate":0.20,  # P(neurologic impairment | CS with complications; GBD 2019)
}
...
class Costs:
    ...
    v3 additions: poc, soc_work, iufd, preterm, term_del, nicu.
    ...
    poc:     float = 50.00;        poc_sd:     float = 10.00          # POC treponemal screen (v3)
    soc_work: float = 500.00;      soc_work_sd: float = 125.00        # Social worker / case mgmt (v3)
    ...
    iufd:    float = 13_049 * CPI; iufd_sd:   float = std2(10_742, 20_141) * CPI  # IUFD ≥28w (v3)
    preterm: float = 37_780 * CPI; preterm_sd: float = std2(26_855, 53_709) * CPI # Preterm del (v3)
    term_del: float = 13_828 * CPI; term_del_sd: float = std2(6_714, 26_855) * CPI # Normal term (v3)
    ...
    nicu:    float = 50_000.0;      nicu_sd:   float = 10_000.0                    # CS comp NICU (v3)
...
def _infant_markov_lifetime(
    ...
    # v3: age-specific background mortality from US Life Table 2021 (replaces fixed mu_bg)
    ...
    def _run(S0: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        v3: uses age-specific mortality mu_t = LIFE_TABLE_QX[t] instead of fixed mu_bg.
        """
...
def _icost(d, cs, sf_cost, sc_b, sc_e, p_act, p_sf, p_id,
    ...
    v2: p_id reduces effective extra screens; mk_lt_cost_saving from infant Markov.
    v3: adds poc (screen cost), soc_work (case mgmt), separates iufd/sb, preterm savings, nicu.
    """
    extra   = max(sc_e * p_id - sc_b * p_id, 0.0) * cohort   # additional screens vs usual care
    p_sn    = max(1.0 - p_act - p_sf, 0.0)
    p_fp    = p_sn * (1.0 - spec)
    n_tx    = extra * p_act * sens * p_adeq                    # true positives detected+treated
    if treat_fp:
        n_tx = n_tx + extra * p_fp * p_adeq
    # Program costs: poc screen + confirmatory RPR/FTA + treatment + social worker + admin + serofast
    test    = (extra * cs["poc"]                                        # v3: POC screen
               + extra * cs["rpr"]                                      # confirmatory RPR
               + extra * (p_act * sens + p_fp) * cs["fta"])             # FTA on positives
    prog    = (test
               + n_tx * (cs["pen"] + cs["soc_work"])                   # v3: social worker per case
               + extra * cs["staff"]
               + sf_cost)
    # Outcome savings — v3 changes:
    #   (a) iufd_subset costs at co.iufd; remaining stillbirths cost at co.sb
    #   (b) preterm births now generate cost savings (preterm - term delivery)
    #   (c) CS complicated cases save nicu cost in addition to cs_wu
    n_iufd    = d["iufd_subset"].astype(float)
    n_sb_term = np.maximum(d["stillbirth"].astype(float) - n_iufd, 0.0)
    sav     = (
        n_iufd      * cs["iufd"]                     +   # v3: IUFD at lower cost
        n_sb_term   * cs["sb"]                        +   # v3: term stillbirths only
        d["neonatal_death"] * cs["nnd"]               +
        d["lbw"]            * cs["lbw_hs"]            +
        d["preterm"]        * cs["preterm"]            +   # v3: preterm delivery cost saving
        d["cs_comp"]   * (cs["cs_wu"] + cs["nicu"])   +   # v3: NICU for CS complicated
...
def _det_icost(p_act, p_sf, p_id, sc_b, sc_e, sens, spec,
    ...
    v2: accepts p_id and explicit serofast parameters.
    v3: uses life-table mortality; adds iufd_subset, preterm, nicu, poc, soc_work to costs.
    ...
    def mean_arm(sc):
        ...
        lbw_cond  = p_eff * lbw_tx_c + (1.0 - p_eff) * lbw_unt_c
        pt_unt_c  = min(UNT_ABS["preterm"] / max(1.0 - sb_unt, eps), 1.0)
        pt_tx_c   = min(UNT_ABS["preterm"] * TX_RR["preterm"]["rr"] / max(1.0 - sb_tx, eps), 1.0)
        pt_cond   = p_eff * pt_tx_c + (1.0 - p_eff) * pt_unt_c
    ...
    # Infant Markov DALYs — v3: uses life-table age-specific mortality
    T = max(int(LE), 1)
    ...
    def _det_markov_per_case(p_h0, p_m0, p_s0):
        """Deterministic per-case Markov run — v3: age-specific life-table mortality."""
        S = np.array([p_h0, p_m0, p_s0, 0.0])
        d_acc = 0.0; c_acc = 0.0
        for t in range(T):
            disc = (1.0 + r) ** (-t)
            mu_t = lt_qx(t)                                # v3: age-specific qx
            d_acc += (S[1] * dw_m + S[2] * dw_s) * disc
            c_acc += (S[1] * cm   + S[2] * cs_)  * disc
            S_new = np.zeros(4)
            S_new[0] += S[0] * (1 - mu_t)
            S_new[3] += S[0] * mu_t
            remain_mild = max(1 - mu_t - q, 0.0)
            S_new[1] += S[1] * remain_mild
            S_new[2] += S[1] * q
            S_new[3] += S[1] * mu_t
            S_new[2] += S[2] * (1 - mu_t)
    ...
    # Program costs — v3: add poc and soc_work
    ...
    prog    = test + n_tx * (co.pen + co.soc_work) + extra * co.staff
    prog   += cohort * p_sf * p_trepo_sf * (co.rpr + co.sf_wu + p_ux_sf * co.pen)
    # Outcome savings — v3: separate iufd/sb, add preterm and nicu
    n_iufd    = dlt["iufd_subset"]
    n_sb_term = max(dlt["stillbirth"] - n_iufd, 0.0)
    sav = (
        n_iufd    * co.iufd                           +   # v3: IUFD at lower cost
        n_sb_term * co.sb                             +   # term stillbirths
        dlt["neonatal_death"] * co.nnd                +
        dlt["lbw"]            * co.lbw_hs             +
        dlt["preterm"]        * co.preterm             +   # v3: preterm cost saving
        dlt["cs_comp"]   * (co.cs_wu + co.nicu)       +   # v3: NICU for CS comp
        dlt["cs_uncomp"] * co.cs_wu
        + mk_cst
    )
    return float(prog - sav), float(dal)


def _markov_annual_seq(p_h0: float, p_m0: float, p_s0: float, n_steps: int) -> np.ndarray:
    """
    Run the deterministic infant Markov for n_steps annual cycles.
    Returns an array of shape (n_steps,) giving the **undiscounted** annual medical
    cost per prevented CS case at each cycle t = 0, 1, …, n_steps−1.

    Used by budget_impact_table for Option-B triangular cohort accumulation:
    BIA year `yr` sums seq[0:yr], representing contributions from yr annual cohorts
    each offset by one additional Markov cycle:
        cohort born in year 1  → currently at cycle yr−1
        cohort born in year 2  → currently at cycle yr−2
        ...
        cohort born in year yr → currently at cycle 0
    Total Markov saving in BIA year yr = cs_cases × sum(seq[0:yr]).
    """
    cm  = INFANT_MK["cost_mild_ann"]["mu"]
    cs_ = INFANT_MK["cost_sev_ann"]["mu"]
    q   = INFANT_MK["q_progress"]
    S   = np.array([p_h0, p_m0, p_s0, 0.0], dtype=float)
    out = np.zeros(n_steps)
    for t in range(n_steps):
        mu_t    = lt_qx(t)                      # age-specific qx from US Life Table 2021
        out[t]  = S[1] * cm + S[2] * cs_        # undiscounted annual cost this cycle
        S_new   = np.zeros(4)
        S_new[0] = S[0] * (1.0 - mu_t)
        S_new[3] = S[0] * mu_t
        remain   = max(1.0 - mu_t - q, 0.0)
        S_new[1] = S[1] * remain
        S_new[2] = S[1] * q + S[2] * (1.0 - mu_t)
        S_new[3] += S[1] * mu_t + S[2] * mu_t + S[3]
        S = S_new
    return out
...
def budget_impact_table(
    ...
    ──────────────────────
    1. (1−TX_RR) factor applied to all prevented-outcome counts — previously missing,
       causing preterm/LBW cases to be ~2× overstated.
    2. sc_b (passed-in baseline coverage) is now used; previously silently recomputed
       internally from ges_eff(), ignoring any sidebar override.
    3. Serofast workup cost is now incremental (extra × p_sf × p_trepo_sf),
       not applied to the full annual_vol population.
    4. Preterm savings are net of the term delivery that still occurs:
       (co.preterm − co.term_del) per case.  co.term_del was defined but unused.
    5. treat_fp parameter added — charges treatment cost for seronegative false positives
       when the sidebar flag is enabled.
    6. Option-B Markov accumulation: each BIA year accumulates savings from ALL cohorts
       born up to that point, each at its correct Markov cycle.  Replaces the prior
       approach of stamping the same full 78-year lifetime NPV in every row.
       Undiscounted per ISPOR BIA good-practice guidance.
    """
    co        = Costs()
    _, tx_eff = ges_eff()                                     # tx_eff from strata; sc_b used directly

    # ── Program costs (constant each year; uptake ramp not modelled) ───────────
    extra  = max(sc_e * p_id - sc_b * p_id, 0.0) * annual_vol   # fix 2: use sc_b
    p_sn   = max(1.0 - p_act - p_sf, 0.0)
    p_fp   = p_sn * (1.0 - spec)
    n_tp   = extra * p_act * sens * p_adeq                        # true-positive treated cases
    n_fp   = extra * p_fp * p_adeq if treat_fp else 0.0           # fix 5: treat_fp flag

    prog_yr = (
        extra   * co.poc                                          # POC treponemal screen
        + extra * co.rpr                                          # confirmatory RPR (all screens)
        + extra * (p_act * sens + p_fp) * co.fta                 # FTA (POC-positive only)
        + (n_tp + n_fp) * (co.pen + co.soc_work)                 # treatment + case management
        + extra * co.staff                                        # ED admin / nursing time
        + extra * p_sf * p_trepo_sf                              # fix 3: incremental serofast
              * (co.rpr + co.sf_wu + p_ux_sf * co.pen)
    )

    # ── Cases prevented per year ───────────────────────────────────────────────
    # p_eff_delta = incremental probability of detection + treatment per syphilis+ woman.
    # Each prevented-outcome count multiplies by (1 − TX_RR) = actual risk reduction
    # per treated case.  fix 1: previously this factor was missing entirely.
    p_eff_delta = (sc_e - sc_b) * p_id * sens * p_adeq * tx_eff  # fix 2: sc_b not sc_uc

    def _prev(outcome):
        return annual_vol * p_act * p_eff_delta * UNT_ABS[outcome] * (1.0 - TX_RR[outcome]["rr"])

    cs_comp_yr   = _prev("cs_any") * prop_symp
    cs_uncomp_yr = _prev("cs_any") * (1.0 - prop_symp)
    sb_yr        = _prev("stillbirth")
    nnd_yr       = _prev("neonatal_death")
    lbw_yr       = _prev("lbw")
    preterm_yr   = _prev("preterm")

    # IUFD is the late-gestation subset of stillbirths (fix 1 already in sb_yr)
    iufd_yr    = sb_yr * prop_late
    sb_term_yr = max(sb_yr - iufd_yr, 0.0)

    # ── Immediate (birth-year) outcome savings — constant each year ────────────
    # fix 4: preterm saving is NET of the term delivery that still occurs
    sav_yr_imm = (
        iufd_yr    * co.iufd                          # IUFD at lower delivery cost
        + sb_term_yr * co.sb                          # term stillbirths
        + nnd_yr   * co.nnd                           # neonatal death avoided
        + lbw_yr   * co.lbw_hs                        # LBW hospitalisation avoided
        + preterm_yr * (co.preterm - co.term_del)     # fix 4: net preterm saving
        + cs_comp_yr   * (co.cs_wu + co.nicu)         # CS complicated: workup + NICU
        + cs_uncomp_yr * co.cs_wu                     # CS uncomplicated: workup only
    )

    # ── Option-B Markov accumulation ───────────────────────────────────────────
    # Pre-compute the undiscounted annual cost at each Markov cycle t = 0…n_years−1
    # for CS complicated and CS uncomplicated separately.
    p_sev = INFANT_MK["p_severe_cs_comp"]["m"]
    p_mc  = INFANT_MK["p_mild_cs_comp"]["m"]
    p_mu  = INFANT_MK["p_mild_cs_uncomp"]["m"]
    mk_seq_comp   = _markov_annual_seq(max(1-p_sev-p_mc, 0.0), p_mc, p_sev, n_years)
    mk_seq_uncomp = _markov_annual_seq(max(1-p_mu, 0.0),       p_mu, 0.0,   n_years)

    # ── Build year-by-year table ───────────────────────────────────────────────
    rows, cum = [], 0.0
    for yr in range(1, n_years + 1):
        # Triangular cohort accumulation (undiscounted, per ISPOR BIA guidance):
        # In BIA year `yr`, there are `yr` cohorts of prevented CS cases.
        # Cohort born in year k is now at Markov cycle (yr − k), k = 1…yr.
        # → sum mk_seq[0 : yr]  (cycle 0 from the newest cohort,
        #                         cycle yr−1 from the oldest cohort)
        mk_sav_yr = (
            cs_comp_yr   * mk_seq_comp[:yr].sum()
            + cs_uncomp_yr * mk_seq_uncomp[:yr].sum()
        )
        sav_yr = sav_yr_imm + mk_sav_yr
        net_yr = prog_yr - sav_yr
        cum   += net_yr
        rows.append({
            "Year":                  yr,
            "Program cost ($)":      prog_yr,
            "Immediate savings ($)": sav_yr_imm,
            "Markov savings ($)":    mk_sav_yr,
    ...
    # Return Year-1 savings/net for the KPI strip; full df for the table and chart
    return df, prog_yr, df.loc[0, "Outcome savings ($)"], df.loc[0, "Net impact ($)"], cs_comp_yr + cs_uncomp_yr
...
def fig_markov_states(r_disc: float, LE: float,
    ...
    def _occ(S0):
        history = [S0.copy()]
        S = S0.copy()
        for t in range(T - 1):                          # v3: t used for lt_qx lookup
            mu_t = lt_qx(t)                             # v3: age-specific mortality
            S_new = np.zeros(4)
            S_new[0] += S[0] * (1 - mu_t); S_new[3] += S[0] * mu_t
            remain = max(1 - mu_t - q, 0.0)
            S_new[1] += S[1] * remain; S_new[2] += S[1] * q; S_new[3] += S[1] * mu_t
            S_new[2] += S[2] * (1 - mu_t); S_new[3] += S[2] * mu_t; S_new[3] += S[3]
...
with st.sidebar:
    ...
    with st.expander("CS Natural History (calibration)", expanded=False):
        st.caption("These rates from the clinical literature are used to cross-check Markov "
                   "initial-state parameters.  Adjust here to explore calibration sensitivity.")
        cs_cure_ui  = st.slider("P(cure | CS + early treatment)",
                                0.80, 1.00, INFANT_MK["cs_early_cure_rate"], 0.01)
        cs_late_ui  = st.slider("P(late complications | untreated CS)",
                                0.05, 0.50, INFANT_MK["cs_late_manifest_rate"], 0.01)
        cs_neuro_ui = st.slider("P(neuro disorder | CS with complications)",
                                0.05, 0.50, INFANT_MK["cs_neuro_disorder_rate"], 0.01)
    # Push user overrides back into INFANT_MK
...
# ─── Top KPI strip ──────────────────────────────────────────────────────────
st.title("🏥 ED Universal Syphilis Screening — CEA  (v3)")
st.caption(
    f"Baseline effective coverage (strata-weighted): **{sc_uc_eff:.1%}** → "
    f"Enhanced: **{sc_e:.0%}**  |  P(pregnancy identified): **{p_id:.0%}**  |  "
    f"Preset: **{preset}**  |  v3: life-table Markov · POC cost · social worker · IUFD split · preterm savings · NICU"
)
...
with tabs[0]:
    ...
    st.download_button("⬇ Download PSA iteration data (CSV)", csv,
                       "syphilis_ed_psa_v3.csv", "text/csv")
...
with tabs[1]:
    ...
    st.info(
        "**Audience: ED administrators and CFOs.**  "
        "Deterministic projection using mean parameter values. "
        "Program costs are constant each year (flat uptake assumed). "
        "Outcome savings grow year-on-year as each annual cohort of prevented CS cases "
        "accumulates Markov healthcare cost savings (Option B: triangular cohort accumulation, "
        "undiscounted per ISPOR BIA guidance). "
        "Perspective = hospital / payer; VSL not included."
    )
    ...
    df_bi, prog_yr, sav_yr1, net_yr1, cs_yr = budget_impact_table(
        ann_vol, p_act, p_sf, p_id, sc_uc_eff, sc_e,
        sens, spec, p_adeq, prop_symp, prop_late,
        p_trepo_sf, p_ux_sf, bool(treat_fp),
        n_yrs,
    )
    ...
    with b2:
        ...
        m1.metric("Annual program cost",         f"${prog_yr:,.0f}",
                  help="Constant each year (flat uptake)")
        m2.metric("Year-1 outcome savings",      f"${sav_yr1:,.0f}",
                  help="Immediate + Markov savings for Year 1 only")
        m3.metric("Year-1 net impact",           f"${net_yr1:,.0f}",
                  delta="Cost-saving ✓" if net_yr1 < 0 else "Net cost",
                  delta_color="normal" if net_yr1 < 0 else "inverse",
                  help="Savings grow in later years as Markov cohorts accumulate")
        m4.metric("CS cases prevented/yr",       f"{cs_yr:.1f}")

    st.pyplot(fig_budget_bars(df_bi), use_container_width=True)

    # Display table — format monetary columns; show new Markov breakdown columns
    disp_bi = df_bi.copy()
    for col in ["Program cost ($)", "Immediate savings ($)", "Markov savings ($)",
                "Outcome savings ($)", "Net impact ($)", "Cumulative net ($)"]:
    ...
    st.caption(
        "**Markov savings** grow each year because each new annual cohort of prevented CS cases "
        "adds its Year-0 healthcare cost saving, while prior cohorts contribute their Year-1, "
        "Year-2, … savings.  By Year 5, five cohorts are active simultaneously at staggered "
        "Markov ages.  All Markov values are undiscounted (ISPOR BIA guidance)."
    )

    nns = ann_vol / max(cs_yr, 0.001)
    net_final = df_bi.loc[df_bi["Year"] == n_yrs, "Net impact ($)"].iloc[0]
    st.markdown(f"""
    | Metric | Value |
    |--------|-------|
    | Number needed to screen (NNS) to prevent 1 CS case | **{nns:,.0f}** |
    | Year-1 cost per CS case prevented | **${max(net_yr1, 0)/max(cs_yr, 0.001):,.0f}** |
    | Year-{n_yrs} cost per CS case prevented | **${max(net_final, 0)/max(cs_yr, 0.001):,.0f}** |
    """)
...
with tabs[3]:
    ...
    **States:**
    ...
    with st.expander("State transition parameters"):
        st.json({
            "Background mortality": "Age-specific (US Life Table 2021) — replaces fixed mu_bg in v3",
    ...
    with st.expander("US Life Table 2021 — background mortality used in Markov (v3 new)"):
        ages_lt = list(range(len(LIFE_TABLE_QX)))
        fig_lt, ax_lt = plt.subplots(figsize=(9, 3.5))
        ax_lt.semilogy(ages_lt, LIFE_TABLE_QX, color="steelblue", lw=2)
        ax_lt.axhline(INFANT_MK["mu_bg"], color="darkorange", ls="--", lw=1.5,
                      label=f"Former fixed mu_bg = {INFANT_MK['mu_bg']:.3f}")
        ax_lt.set_xlabel("Age (years)"); ax_lt.set_ylabel("Annual mortality probability (log scale)")
        ax_lt.set_title("US Life Table 2021 — Age-Specific qx", fontweight="bold")
        ax_lt.legend(fontsize=8); ax_lt.grid(alpha=0.15)
        ax_lt.spines[["top","right"]].set_visible(False)
        st.pyplot(fig_lt, use_container_width=True)
        st.caption(
            "At birth (age 0) qx = 0.00586, falling to ~0.00027 by age 2, then "
            "rising monotonically from adulthood. The former fixed mu_bg = 0.003 "
            "underestimates early infant mortality and overestimates adult mortality — "
            "switching to the life table shifts the DALY accumulation curve correctly."
        )

    with st.expander("CS Natural History — Calibration Cross-Check (v3 new)"):
        cr   = INFANT_MK["cs_early_cure_rate"]
        lm   = INFANT_MK["cs_late_manifest_rate"]
        nd   = INFANT_MK["cs_neuro_disorder_rate"]
        # Implied Markov initial-state check for untreated complicated CS
        implied_severe = lm * nd        # P(neuro | untreated) = P(complication) × P(neuro|complication)
        implied_mild   = lm * (1 - nd) # P(mild | untreated)
        implied_healthy = 1 - implied_severe - implied_mild
        st.markdown(f"""
        | Natural history parameter | Value |
        |---------------------------|-------|
        | P(cure | early treatment) | **{cr:.0%}** |
        | P(late complications | untreated) | **{lm:.0%}** |
        | P(neurologic disorder | complications) | **{nd:.0%}** |

        **Calibration check — implied initial-state distribution for *untreated* CS complicated:**

        | State | Implied from natural history | Markov prior (sidebar) |
        |-------|------------------------------|------------------------|
        | Healthy | {implied_healthy:.0%} | {max(1 - p_sev_ui - p_mc_ui, 0):.0%} |
        | Mild sequelae | {implied_mild:.0%} | {p_mc_ui:.0%} |
        | Severe sequelae | {implied_severe:.0%} | {p_sev_ui:.0%} |

        {'✅ Natural history estimates are broadly consistent with Markov priors (within ±10pp).'
          if abs(implied_severe - p_sev_ui) < 0.10 and abs(implied_mild - p_mc_ui) < 0.10
          else '⚠️ Note: natural history estimates diverge from Markov priors by >10 percentage points — consider revising one or the other.'}
        """)
...
with tabs[5]:
    ...
    with st.expander("v3 additions (this version)", expanded=True):
        st.markdown("""
        | Addition | Detail |
        |----------|--------|
        | POC screen cost | $50/screen added to program cost; applies to all incremental screens |
        | Social worker cost | $500/treated case added to program cost; ED case management |
        | US Life Table 2021 | Age-specific qx replaces fixed mu\\_bg=0.003 in infant Markov |
        | Separate IUFD cost | IUFD ≥28w ($13,049 CPI-adj) vs term stillbirth ($141,792); prevents overestimation of savings |
        | Preterm savings (net) | Net of term delivery cost: ($37,780 − $13,828) CPI-adj per case |
        | Term delivery cost | $13,828 CPI-adj; used to net preterm savings |
        | NICU for CS complicated | $50,000 per CS complicated infant added to savings alongside cs\\_wu |
        | CS natural history | cs\\_early\\_cure\\_rate, cs\\_late\\_manifest\\_rate, cs\\_neuro\\_disorder\\_rate for Markov calibration |
        | BIA: (1−TX_RR) fix | Prevented-case counts now multiplied by treatment RR reduction; previously missing, inflating preterm/LBW savings ~2× |
        | BIA: sc\\_b fix | Baseline coverage now uses passed-in sc\\_b; previously recomputed internally ignoring sidebar |
        | BIA: serofast incremental | Serofast workup cost applied to incremental screens only (extra × p\\_sf), not full population |
        | BIA: Option-B Markov | Triangular cohort accumulation: Year yr sums cycles 0…yr−1 across yr cohorts; undiscounted per ISPOR |
        | BIA: treat\\_fp wired | False-positive treatment cost now propagates to BIA when sidebar flag is enabled |
        """)

    with st.expander("v2 structural changes", expanded=False):
        st.markdown("""
        | Change | Detail |
        |--------|--------|
